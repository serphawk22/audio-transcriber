...
def get_ai_tutor_response(message, base_language, target_language, history, weak_words=None):
    """Get AI tutor response using Gemini, optionally focusing on weak vocabulary"""
...
Respond as a warm, interactive friend. Keep it natural and engaging."""
    ...
    try:
        response = gemini_service.gemini_service.model.generate_content(prompt)
...
{{
  ...
  "original": "{text}"
    ...
    try:
        response = gemini_service.gemini_service.model.generate_content(
            prompt,
...
def get_practice_phrase(base_language, target_language, exclude=None, focus_word=None):
    ...
    exclude_str = ""
    if exclude:
        # If focusing on a word, remove it from exclusion if it was accidentally there
    ...
    prompt = f"""Generate a simple, common {target_lang_name} phrase for a beginner to practice speaking.{exclude_str}{focus_str}
...
Ensure the phrase is natural and different from very basic greetings like "Hello" or "How are you" if they are already in the list.
